...
class blackjack:
    def __init__(self, master=None):
        ...
        self.dealerpoints.configure(background="#194d33",text='0',textvariable=self.dealerpoint)
        self.dealerpoints.grid(column=0, row=0)

        self.playerpoints = ttk.Label(self.cardsframe)
        self.playerpoint = tk.StringVar(value=0)
        self.playerpoints.configure(background="#194d33",text='0',textvariable=self.playerpoint)
    ...
    def playerbust(self):
        ...
        self.hitbtn["state"] = DISABLED
        self.doublebtn["state"] = DISABLED
        self.playerpoint.set(">21 :(" + str(gamedata1.player.points))
        self.dealerpoint.set(">21 :(" + str(gamedata1.dealer.points))
    ...
    def compervalue(self):
        ...
        if(gamedata1.dealer.hasbj and gamedata1.player.hasbj):
            self.playerpoint.set("DEALER AND PLAYER BLACKJACK!!!!! "+ str(gamedata1.player.points))
            self.dealerpoint.set("DEALER AND PLAYER BLACKJACK!!!!! "+ str(gamedata1.dealer.points))
            player1.monney = player1.monney + player1.bet.get()
        
        elif(gamedata1.player.hasbj):
            self.playerpoint.set("BLACKJACK!!!!! "+ str(gamedata1.player.points))
            self.dealerpoint.set("BLACKJACK!!!!! "+ str(gamedata1.dealer.points))
            player1.monney = player1.monney + player1.bet.get()*2.5
        elif(gamedata1.dealer.hasbj):
            self.playerpoint.set("DEALER BLACKJACK!!!!! "+ str(gamedata1.player.points))
            self.dealerpoint.set("DEALER BLACKJACK!!!!! "+ str(gamedata1.dealer.points))
            player1.monney = player1.monney + player1.bet.get()

        elif(gamedata1.dealer.points > 21):
            self.playerpoint.set("Dealer bust " + str(gamedata1.player.points))
            self.dealerpoint.set("Dealer bust "+ str(gamedata1.dealer.points))
            player1.monney = player1.monney + player1.bet.get()*2

        elif(gamedata1.dealer.points > gamedata1.player.points):
            self.playerpoint.set("Dealer points > Player points " + str(gamedata1.player.points))
            self.dealerpoint.set("Dealer points > Player points "+ str(gamedata1.dealer.points))
